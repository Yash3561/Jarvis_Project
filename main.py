...
# --- NEW: Import the file writer tool ---
from file_writer_tool import write_to_file
...
class ChatWindow(QWidget):
    ...
    def __init__(self, agent_instance):
        ...
        # --- NEW: Store the last assistant response ---
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Jarvis Co-Pilot"); self.setGeometry(300, 300, 700, 600); self.setWindowFlags(Qt.WindowType.WindowStaysOnTopHint | Qt.WindowType.Tool); self.setStyleSheet("background-color: #161B22;"); self.layout = QVBoxLayout(self); self.layout.setContentsMargins(10, 10, 10, 10); self.layout.setSpacing(10); self.chat_display = QTextBrowser(self); self.chat_display.setOpenExternalLinks(True); self.chat_display.setStyleSheet("QTextBrowser { background-color: #0D1117; color: #C9D1D9; border: 1px solid #30363D; border-radius: 6px; font-family: 'Segoe UI', sans-serif; font-size: 14px; padding: 15px; } a { color: #58A6FF; } p { margin-bottom: 10px; } ul, ol { margin-left: 20px; } li { margin-bottom: 5px; } code { background-color: #2D333B; color: #A6ACCD; border-radius: 4px; padding: 2px 5px; font-family: 'Fira Code', monospace; } pre { background-color: #010409; border: 1px solid #30363D; border-radius: 5px; padding: 10px; margin: 5px 0; font-family: 'Fira Code', monospace; color: #A6ACCD; white-space: pre-wrap; }"); self.layout.addWidget(self.chat_display); input_layout = QHBoxLayout(); self.input_box = QLineEdit(self); self.input_box.setPlaceholderText("Ask Jarvis..."); self.input_box.setStyleSheet("QLineEdit { background-color: #0D1117; color: #C9D1D9; border: 1px solid #30363D; border-radius: 6px; padding: 10px; } QLineEdit:focus { border: 1px solid #58A6FF; }"); self.input_box.returnPressed.connect(self.handle_input); input_layout.addWidget(self.input_box); self.listen_button = QPushButton("🎤", self); self.listen_button.setFixedSize(40, 40); self.listen_button.setStyleSheet("QPushButton { background-color: #21262D; color: #C9D1D9; border: 1px solid #30363D; border-radius: 20px; } QPushButton:hover { background-color: #30363D; }"); self.listen_button.clicked.connect(self.toggle_listening); input_layout.addWidget(self.listen_button); self.layout.addLayout(input_layout); self.add_message_to_display("system", "Jarvis is ready. Type or press the mic to speak.")
    ...
    def route_command(self, text):
        ...
        # --- NEW: Command route for saving files ---
        save_keywords = ['save this', 'save the code', 'write to file', 'save that']
        if any(keyword in text.lower() for keyword in save_keywords):
            # Regex to find a filename like 'test.py' or 'my-file.txt'
            filename_match = re.search(r'([\w\.-]+\.\w+)', text)
            if filename_match:
                filename = filename_match.group(1)
                threading.Thread(target=self.run_write_file_task, args=(filename,)).start()
                return True
    ...
    # --- NEW: Task for the file writer command ---
    def run_write_file_task(self, filename):
        if not self.last_assistant_response:
            self.response_received.emit("I don't have anything to save. Please ask me to generate some content first.")
            return

        self.add_message_to_display("system", f"Saving content to '{filename}'...")
        
        # Extract only the code from the last response if it's in a block
        code_match = re.search(r'```(?:python\n)?(.*)```', self.last_assistant_response, re.DOTALL)
        content_to_save = code_match.group(1).strip() if code_match else self.last_assistant_response
        
        result = write_to_file(filename, content_to_save)
        self.response_received.emit(result)

    def on_agent_response(self, response):
        # --- NEW: Store the raw response before displaying it ---
    ...
    # (All audio methods are unchanged)
